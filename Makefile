...
MATRIX_SRC := $(SRCDIR)/matrix/matrix.c
MATRIX_TEST_SRC := $(SRCDIR)/matrix/matrix-pool.c
...
MATRIX_OBJ := $(BINDIR)/matrix.o
MATRIX_TEST_OBJ := $(BINDIR)/matrix_test.o
...
MATRIX_TEST_TARGET := $(BINDIR)/matrix_test

# Build rules
all: barrier_test vector_test countdown_test thread_pool_test matrix_test
...
matrix_test: $(MATRIX_TEST_TARGET)

$(MATRIX_TEST_TARGET): $(MATRIX_TEST_OBJ) $(MATRIX_OBJ) $(THREAD_POOL_OBJ) $(SOT_BARRIER_OBJ)
	$(CC) $(CFLAGS) -o $@ $^
...
$(MATRIX_TEST_OBJ): $(MATRIX_TEST_SRC)
	$(CC) $(CFLAGS) -c -o $@ $<

clean_obj:
	rm -f $(SOT_BARRIER_OBJ) $(BARRIER_TEST_OBJ) $(VECTOR_TEST_OBJ) $(VECTOR_UTIL_OBJ) $(COUNTDOWN_OBJ) $(COUNTDOWN_TEST_OBJ) $(THREAD_POOL_OBJ) $(THREAD_POOL_TEST_OBJ) $(MATRIX_OBJ) $(MATRIX_TEST_OBJ)
clean:
	rm -f $(BARRIER_TEST_TARGET) $(VECTOR_TEST_TARGET) $(COUNTDOWN_TEST_TARGET) $(THREAD_POOL_TEST_TARGET) $(MATRIX_TEST_TARGET)
	rm -f $(SOT_BARRIER_OBJ) $(BARRIER_TEST_OBJ) $(VECTOR_TEST_OBJ) $(VECTOR_UTIL_OBJ) $(COUNTDOWN_OBJ) $(COUNTDOWN_TEST_OBJ) $(THREAD_POOL_OBJ) $(THREAD_POOL_TEST_OBJ) $(MATRIX_OBJ) $(MATRIX_TEST_OBJ)
